...
import '../home/homepage_screen.dart'; // Đảm bảo import đúng đường dẫn
...
class _OnboardingFlowScreenState extends State<OnboardingFlowScreen> {
  ...
  final AuthService _authService = AuthService(); // Singleton
  ...
  Future<void> _fetchOptionsFromServer() async {
    try {
      final options = await _authService.getSurveyOptions();
  ...
  void _onNextPressed() {
    ...
    switch (_currentPage) {
      ...
      case 2:
        ...
        if (isValid) _formData['age'] = int.tryParse(_ageController.text) ?? 20;
        break;
      case 5:
    ...
    // Nếu ở trang chọn Goal (Trang 6) -> Chuyển sang trang 7 (AI Analysis) thì gọi API
    if (_currentPage == 6) {
      _runAiAnalysis();
    }
  ...
  Future<void> _runAiAnalysis() async {
    ...
    // Gọi Service (đã được sửa trả về full data gồm cả nutrition & filteredRecipes)
    final result = await _authService.submitSurvey(dataToSend);
    ...
    if (result != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text("Lỗi kết nối Server. Vui lòng thử lại!"),
            backgroundColor: kColorError),
      );
    }
  ...
  // --- LOGIC CHUYỂN TRANG HOME ---
  Future<void> _submitProfile() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('isSurveyDone', true);

    if (mounted) {
      // Lấy dữ liệu đã lọc sẵn để truyền qua Home
      final List<dynamic>? filteredRecipes = _aiData?['filteredRecipes'];
      final Map<String, dynamic>? nutritionData = _aiData?['nutrition'];

      Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(
          builder: (context) => HomePage(
            // Truyền dữ liệu qua để Home hiển thị luôn không cần load lại
            // Lưu ý: HomePage của bạn cần update constructor để nhận 2 biến này
      ...
      bottomNavigationBar: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed:
                  (_currentPage == 7 && _isAnalyzing) ? null : _onNextPressed,
              ...
              child: Text(
                _currentPage == _totalPages - 1 ? 'Vào Bếp' : 'Next',
                style: GoogleFonts.interTight(
                    fontWeight: FontWeight.w600, fontSize: 16.0),
              ),
            ),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Trước khi bắt đầu, hãy cá nhân hóa trải nghiệm của bạn.',
              textAlign: TextAlign.center,
  ...
  Widget _buildPage_BodyStats() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _buildHeader("Chỉ số cơ thể", "Nhập chiều cao và cân nặng để tính BMI.",
            isDark: false),
        const SizedBox(height: 32.0),
        _buildNumberSlider(
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _exclusionOptions.isEmpty
            ...
            : _buildChipGroup(_exclusionOptions, _formData['exclusions'],
                (isSelected, option) {
                setState(() {
                  isSelected
        ...
        _buildRadioGroup(
            ...
            onChanged: (value) => setState(() {
                  ...
                  if (!value.toLowerCase().contains('vegetarian') &&
                      !value.toLowerCase().contains('vegan') &&
                      !value.toLowerCase().contains('chay')) {
    ...
    return SingleChildScrollView(
      ...
      child: Form(
          ...
          child:
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _buildHeader(
                "Mục tiêu dinh dưỡng?", "Hãy cho biết mục tiêu chính của bạn."),
            const SizedBox(height: 32.0),
  ...
  Widget _buildPage_AiAnalysis() {
    ...
    return _aiData == null
        ...
        : SingleChildScrollView(
            padding: const EdgeInsets.all(24.0), child: _buildAnalysisReport());
  }

  Widget _buildAnalysisReport() {
    if (_aiData == null || _aiData!['nutrition'] == null) return const SizedBox();
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _buildHeader(
          "AI Nutrition Analysis", "Kết quả phân tích dựa trên hồ sơ của bạn."),
      const SizedBox(height: 24.0),
      Container(
          ...
          child: Row(children: [
            ...
            Expanded(
                child: _buildStatItem(
                    "Nước", nutrition['water'] ?? "2L", "/ngày")),
            Expanded(
                child: _buildStatItem(
                    "Calo", "${nutrition['calories'] ?? 0}", "kcal/ngày")),
          ])),
      const SizedBox(height: 24.0),
      ...
      if (isDataEmpty)
        Center(
            child: Padding(
                ...
                child: Text("(Dữ liệu mẫu do AI chưa cung cấp chi tiết macros)",
                    style: GoogleFonts.inter(
                        color: Colors.grey,
                        fontSize: 12,
      ...
      if (foodsToAvoid.isNotEmpty)
        Container(
            ...
            child: Column(children: [
              _buildAnalysisItem(Icons.warning_amber_rounded, 'Nên tránh:',
                  foodsToAvoid, kColorError)
            ])),
  ...
  Widget _buildStatItem(String label, String value, String sub) {
    return Column(children: [
      Text(label,
          style: GoogleFonts.inter(color: kColorSecondaryText, fontSize: 12)),
  ...
  Widget _buildPage_Done() {
    // Đếm số lượng món ăn đã lọc được từ backend
    final int recipeCount =
        (_aiData != null && _aiData!['filteredRecipes'] != null)
            ? (_aiData!['filteredRecipes'] as List).length
            : 0;
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Text('Hoàn tất!',
              style: GoogleFonts.interTight(
                  ...
                  fontWeight: FontWeight.w600)),
          const SizedBox(height: 16.0),
          Text('Hồ sơ đã được lưu. Hãy bắt đầu nấu ăn!',
              style: GoogleFonts.inter(
          ...
          if (recipeCount > 0)
            Container(
              ...
              child: Row(
                ...
                children: [
                  const Icon(Icons.restaurant, color: kColorPrimary, size: 20),
                  const SizedBox(width: 8),
                  Text('Đã tìm thấy $recipeCount món ăn cho bạn!',
                      style: GoogleFonts.inter(
                          color: kColorPrimary, 
                          fontWeight: FontWeight.w600)),
                ],
  ...
  Widget _buildNumberSlider(
    ...
    final int divisions = (maxValue - minValue).toInt() * 2;
    return Column(children: [
